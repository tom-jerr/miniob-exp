 ...
 */
void parse_csv_line(char *line, vector<string> &file_values, const string &terminated, const string &enclosed)
{
  if (!line || *line == '\0') {
    return;
  }

  // ClickBench数据集通常使用Tab分隔符，如果没有指定则默认使用逗号
  char delimiter = terminated.empty() ? '\t' : terminated[0];
  char quote     = enclosed.empty() ? '"' : enclosed[0];
  ...
  while (*current != '\0') {
    ...
    } else if (*current == delimiter && !in_quotes) {
      // 字段分隔符且不在引号内
      *current = '\0';  // 终止当前字段
...
RC LoadDataExecutor::execute(SQLStageEvent *sql_event)
{
  ...
  string        terminated = stmt->terminated();
  string        enclosed   = stmt->enclosed();
  load_data(table, file_name, terminated, enclosed, sql_result);
  return rc;
}
...
void LoadDataExecutor::load_data(
    Table *table, const char *file_name, string terminated, string enclosed, SqlResult *sql_result)
{
  RC           rc = RC::SUCCESS;
